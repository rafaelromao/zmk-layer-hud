...
test-host: ## Python tests for the raw-HID decoder in host/hudfeed.py
	$(PYTHON) -m unittest discover -s host -p '*_test.py' -v

keymap: ## rebuild hud/keymap.json + keymap.js from the keyboards repo ($$KEYBOARDS_REPO)
	$(PYTHON) hud/keymap/build.py

clean:
	rm -rf build hud/keymap.json hud/keymap.js
